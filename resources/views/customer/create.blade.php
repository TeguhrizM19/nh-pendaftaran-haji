<x-layout>
  <div>
    <x-page-title>Form Data Customer</x-page-title>
  </div>
...
document.getElementById("searchInput").addEventListener("input", function () {
    ...
    fetch(`/search-customer?query=${encodeURIComponent(query)}`)
    ...
    .then(data => {
        ...
        if (data.length > 0) {
            data.forEach(customer => {
                ...
                li.dataset.nama = customer.nama;

                li.classList.add("flex", "justify-between", "items-center", "p-4", "hover:bg-blue-100", "cursor-pointer");
                li.innerHTML = `
                    <span>${customer.nama}</span>
                    <div>
                      <a href="/repeat-data-customer/${customer.id}" class="px-3 py-1 bg-[#099AA7] text-white rounded text-sm">🔄 Repeat</a>
                      <a href="/ambil-semua-data-customer/${customer.id}" class="px-3 py-1 bg-blue-600 text-white rounded text-sm">⬆️ Ambil Semua Data</a>
                    </div>
                `;
